...
import { updateRegistrationDetails, saveProgress } from '../../store/registration-store/registration.action';
import { Store } from '@ngrx/store';
import { selectFullRegistrationState, selectRegistrationStep } from '../../store/registration-store/registration.selector';
...
export class JoinProfessionalComponent implements OnInit, OnDestroy {
    ...
    nextStep() {
        if (this.currentStep < 3) {
            this.store.dispatch(saveProgress({ registrationState: { step: this.currentStep + 1, formData: this.formData$, isValid: false, canSubmit: false, isComplete: false } }));
